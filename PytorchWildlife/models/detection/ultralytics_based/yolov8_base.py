...
class YOLOV8Base(BaseDetector):
    ...
    def _load_model(self, weights=None, device="cpu", url=None):
        ...
        if weights:
            ...
            logger.info("Detection model's checkpoint downloading")
            if not os.path.exists("./checkpoints/MDV6b-yolov9c.pt"):
                logger.info("Detection model's checkpoint was not found locally")
                weights = wget.download(url, out="./checkpoints")
                logger.success("Detection model's checkpoint successfully downloaded")
            else:
                logger.info("Detection model's checkpoint was found locally")
                weights = "./checkpoints/MDV6b-yolov9c.pt"
            self.predictor.setup_model(weights)
            logger.success("Detection model is successfully created")
        else:
